...
3. Create new file `src/core/muscle-groups.ts` with:
   ...
   - DEFAULT_MUSCLE_GROUP_CONFIG with 5 groups: Push, Pull, Legs, Core, Arms
   - Push: Pec Sternal, Pec Clavicular, Anterior Deltoid, Lateral Deltoid, Triceps (both)
   - Pull: Lats, Traps (all 3), Posterior Deltoid, Biceps, Erector Spinae, Forearm Flexors, Forearm Extensors
   - Legs: Quads (both), Glutes (both), Hamstrings, Adductors, Gastrocnemius, Soleus
   - Core: Rectus Abdominis, Obliques, Hip Flexors
   - Arms: Empty array (arms already in Push/Pull) - filter this out in default
...
Run: `npm run lint` passes
  ...
  <done>
Schema types exist, DEFAULT_MUSCLE_GROUP_CONFIG has all 26 muscles distributed across 4 groups, validation catches overlap and missing muscles.
  </done>
</task>
...
1. `useEffectiveMuscleGroupConfig(profileId: string | null)`:
   - Uses useProfile to get profile data
   - Returns profile.customMuscleGroups if defined, else DEFAULT_MUSCLE_GROUP_CONFIG
   - Returns { config: MuscleGroupConfig, isLoading: boolean, error: Error | null }
...
Lint passes: `npm run lint`
  ...
  <done>
useMuscleGroups hook exports useEffectiveMuscleGroupConfig and useMuscleGroupMutations, following existing TanStack Query patterns.
  </done>
</task>
...
4. Verify in code:
   ...
   - DEFAULT_MUSCLE_GROUP_CONFIG includes all 26 muscles with no duplicates
   - MAX_GROUPS = 8
...
- Hook provides effective config (custom or default) and save mutations
