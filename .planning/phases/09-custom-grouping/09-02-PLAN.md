...
1. Create `src/ui/components/settings/MuscleGroupEditor.tsx`:
   ...
   - When adding group: use `crypto.randomUUID()` for group id (NOT generateId - crypto.randomUUID provides guaranteed uniqueness required for dnd-kit stable keys), name "New Group", empty muscles array
...
NOTE: Use `crypto.randomUUID()` instead of generateId for new group IDs. The crypto API provides guaranteed uniqueness which dnd-kit requires for stable drag-drop keys. generateId (timestamp + random suffix) could theoretically collide during rapid group creation.
  </action>
  <verify>
TypeScript compiles: `npx tsc --noEmit`
Lint passes: `npm run lint`
Dev server runs: `npm run dev` - visit /settings
  ...
  <done>
MuscleGroupEditor shows all groups with drag-and-drop, inline editing, muscle picker, and reset button. Settings page displays the new Muscle Groups section. Group IDs use crypto.randomUUID() for guaranteed uniqueness.
  </done>
</task>
...
- Group IDs generated with crypto.randomUUID() for dnd-kit stability
</success_criteria>
